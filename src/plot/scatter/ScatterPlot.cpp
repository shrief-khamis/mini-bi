...
ScatterPlot::ScatterPlot(const DataTable& table) {
    setData(table);
}

void ScatterPlot::setData(const DataTable& table) {
    m_x.clear();
    m_y.clear();
    m_xLabel.clear();
    m_yLabel.clear();
...
ScatterPlot::Bounds ScatterPlot::computeDataBounds() const {
    Bounds b;
    if (m_x.empty()) {
        b.isEmpty = true;
        return b;
...
ScatterPlot::Bounds ScatterPlot::paddedBounds(const Bounds& b) {
    if (b.isEmpty)
        return b;
    Bounds out = b;
...
QRectF ScatterPlot::plotRectForCanvas(const QRect& canvasArea) const {
    return QRectF(
...
void ScatterPlot::drawCanvasBackground(QPainter& painter, const QRect& canvasArea) const {
    painter.fillRect(canvasArea, QColor(252, 252, 255));
}

void ScatterPlot::drawEmptyState(QPainter& painter, const QRect& canvasArea) const {
...
void ScatterPlot::drawPlotRegionBackground(QPainter& painter, const QRectF& plotRect) const {
...
void ScatterPlot::drawAxes(QPainter& painter, const QRectF& plotRect) const {
    painter.setPen(QPen(QColor(60, 60, 70), 1.5));
    painter.drawLine(plotRect.bottomLeft(), plotRect.bottomRight());
    painter.drawLine(plotRect.bottomLeft(), plotRect.topLeft());
}

void ScatterPlot::drawGridLines(QPainter& painter, const QRectF& plotRect, const Bounds& dataBounds,
...
void ScatterPlot::drawTickMarksAndLabels(QPainter& painter, const QRectF& plotRect, const Bounds& dataBounds,
...
void ScatterPlot::drawAxisTitles(QPainter& painter, const QRectF& plotRect, const QRect& canvasArea) const {
    QFont font = painter.font();
    font.setBold(true);
    painter.setFont(font);
...
void ScatterPlot::drawPoints(QPainter& painter, const QRectF& plotRect, const Bounds& dataBounds) const {
    painter.setPen(QPen(QColor(52, 88, 158), 1));
    painter.setBrush(QColor(80, 120, 200, 200));
    constexpr double r = 3.5;
    for (std::size_t i = 0; i < m_x.size(); ++i) {
        const QPointF p = dataToPixel(m_x[i], m_y[i], plotRect, dataBounds);
        painter.drawEllipse(p, r, r);
    }
}

QPointF ScatterPlot::dataToPixel(double x, double y, const QRectF& plotRect, const Bounds& b) const {
...
std::vector<double> ScatterPlot::computeTickValues(double minV, double maxV, int maxTicks) {
...
void ScatterPlot::render(QPainter& painter, const QRect& canvasArea) {
