...
/**
 * Line plot: reads numeric pairs from the first two columns of a DataTable,
 * then renders axes, ticks, labels, and a polyline through points in row order.
 */
class LinePlot : public PlotRenderer {
public:
    explicit LinePlot(const DataTable& table);
    ~LinePlot() override = default;
...
private:
    ...
    void drawLineSeries(QPainter& painter, const QRectF& plotRect, const Bounds& dataBounds) const;
